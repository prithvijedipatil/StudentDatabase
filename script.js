...
studentForm.addEventListener('submit', function(event) {
  ...
  if (isEditing) {
    // Edit student
    const editedStudent = database.find(student => student.ID === editStudentId);
    if (editedStudent) {
...
document.addEventListener('click', function(event) {
  if (event.target.classList.contains('delete-btn')) {
    const studentId = parseInt(event.target.dataset.id);
    const studentIndex = database.findIndex(student => student.ID === studentId);
    if (studentIndex !== -1) {
...
searchInput.addEventListener('input', function(event) {
  ...
  const filteredStudents = database.filter(student =>
